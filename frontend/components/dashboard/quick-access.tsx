  ...
  return (
    <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-5">
      {quickLinks.map((link) => (
        <Card
          key={link.id}
          className="quick-access-card overflow-hidden border-0 shadow-md hover:shadow-lg transition-all duration-300 cursor-pointer"
            ...
            <div className={`bg-gradient-to-br ${link.color} p-6 h-full flex flex-col`}>
              ...
              <div className="mt-auto">
                <Button
                  ...
                  disabled={isNavigating === link.id}
                >
                  {isNavigating === link.id ? "Chargement..." : "Accéder →"}
                </Button>
              </div>
            </div>
          </CardContent>
